...
public class Driver_v1_7_R4 implements NDriver {
    ...
    @Override
    public NBTMap getNBT(org.bukkit.inventory.ItemStack item) {
        net.minecraft.server.v1_7_R4.ItemStack nms = item instanceof CraftItemStack ? ((CraftItemStack) item).handle : CraftItemStack.asNMSCopy(item);
        if (nms == null) {
            return NBTMap.EMPTY;
        }
        if (!nms.hasTag()) {
        ...
        return new NBTMapImpl(nms.getTag());
    }
